...
def run_hmc(init_zs,log_like,loglik_func_grad,hmc_stepsize, mass_mat, v_A,v_B,v_cov,rev_A,rev_B, rev_cov, num_iters, leap_steps,rs,callback):
    ...
    # print 'll_new',np.mean(log_like(new_zs)).value*num_iters
    # print 'll', np.mean(log_like(zs)).value*num_iters
    # print 'rev_new', np.mean(log_rev_prob_new_v).value*num_iters
    # print 'log_prob_v', np.mean(log_prob_v).value*num_iters
    # print 'rev_A',rev_A.value
    # print 'rev_B', rev_B.value
    # print 'rev_cov', rev_cov.value
    return zs,L
...
def build_hmc_sampler(loglik_func, D, num_steps,leap_steps):
    ...
    def hmc_sample(params, rs, num_samples,leap_steps, callback=None):
