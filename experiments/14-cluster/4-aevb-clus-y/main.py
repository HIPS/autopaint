...
def plot_latent_centers(encoder,decoder,enc_w,dec_w,train):
    latent_images = encoder(enc_w,train)[0]
    im_clus = kmeans(10)
    im_clus.fit(latent_images)
    centers = im_clus.cluster_centers_
    im_cents = decoder(dec_w,centers)[:,0:784]
    fig = plt.figure(1)
    fig.clf()
...
if __name__ == '__main__':
    ...
    with open('../../../autopaint/mnist_binary_data.pkl') as f:
        N_data, train_im, train_l, test_images, test_labels = pickle.load(f)

    train = np.concatenate((train_im,train_l),axis = 1)
    encoder,decoder,enc_w,dec_w = run_aevb(train)
