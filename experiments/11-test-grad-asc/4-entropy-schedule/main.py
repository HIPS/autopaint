...
def lower_bound(weights,encode,decode_log_like,N_weights_enc,train_images,samples_per_image,latent_dimensions,rs,iter):
...
def run_aevb(train_images):
    ...
    def batch_value_and_grad(weights, iter):
        mod_iter = iter % len(batch_idxs)
        cur_data = train_images[batch_idxs[mod_iter]]

        return lower_bound(weights,encoder,decoder_log_like,N_weights_enc,cur_data,samples_per_image,latent_dimensions,rs,iter)
